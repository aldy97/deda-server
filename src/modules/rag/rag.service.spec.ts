...
`;
  ...
  describe("retrieve", () => {
    it("should retrieve content from file for existing textbook and unit", async () => {
      const result = await service.retrieve("test-book", "u1");
    ...
    it("should return found=false for missing unit", async () => {
      const result = await service.retrieve("test-book", "u99");
      expect(result.found).toBe(false);
      expect(result.content).toBe("");
    });

    it("should return found=false for missing textbook", async () => {
      const result = await service.retrieve("missing-book", "u1");
      expect(result.found).toBe(false);
    });

    it("should accept query parameter without changing result", async () => {
      const result = await service.retrieve("test-book", "u2", "red color");
      expect(result.found).toBe(true);
      expect(result.content).toContain("red");
    });

    it("should retrieve content from DB when available", async () => {
      const prismaMock = {
        unit: {
      ...
      const module: TestingModule = await Test.createTestingModule({
        providers: [
          ...
          {
            provide: ConfigService,
            useValue: { get: jest.fn().mockReturnValue(tempDir) },
          },
          {
      ...
      const dbService = module.get<RagService>(RagService);
      const result = await dbService.retrieve("db-book", "u1");
      expect(result.found).toBe(true);
      expect(result.unitName).toBe("DB Unit");
      expect(result.content).toBe("Content from database.");
    });
  });
